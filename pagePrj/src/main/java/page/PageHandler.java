...
public class PageHandler {
	 ...
	 int  totRecords ;          // 총 레코드 수 	 (테이블로 구하기)
	 int  pageSize ;              //페이지 사이즈  (데이터 건수 , 한 페이지에 보이는 레코드 수 -직접설정)	 	 
	 int  totalPage;             // 총 페이지수: 전체 페이수   (계산) 
	  
	 int  grpSize=5 ;             //그룹사이즈= 한 그룹에 5개씩 보겟다 (직접 설정)
	 int  currentGrp ;  	      // 현재그룹 (계산)
